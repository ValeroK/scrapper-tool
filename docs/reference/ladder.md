...
IMPERSONATE_LADDER: tuple[BrowserTypeLiteral, ...] = (
    "chrome146",   # primary — freshest in curl_cffi 0.x as of 2026-04-30
    "chrome142",    # fallback — validated against 5 production vendor adapters
    "safari260",   # diversification (chrome family disproportionately fingerprinted — see curl_cffi#500)
...
**Bumping the primary**: when `chrome146` starts showing >5% 403 rate in the [`live-canary.yml`](../../.github/workflows/live-canary.yml) workflow, bump it to the next stable `curl_cffi` profile (currently `chrome142`/`chrome146` are the freshest available; promote whichever has stabilised).
