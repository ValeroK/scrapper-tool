...
|---|---|---|---|
| `auto_scrape` *(v1.1.0+; v1.2.0 adds `hostile_only` + `pattern_d_network_idle` + `is_structured`)* | `url, schema_json?, instruction?, model?, browser?, timeout_s?, hostile_only?, hostile_fallback?, pattern_d_network_idle?` | `{pattern_used, pattern_attempts, product?, data?, blocked, hostile_skipped, is_structured, ...}` | **Recommended first tool.** Auto-escalates Pattern A/B/C → D → E1 → E2. Set `hostile_only=True` to skip the A/B/C ladder for recon-classified hostile vendors (saves ~2-3s/call). Set `pattern_d_network_idle=True` for SPA-rendered hostile vendors (Tasca, etc.) where results lazy-load. `hostile_fallback=False` raises rather than escalating to E1/E2 when D fails. Returns `is_structured` (sidecar's success verdict). |
