...
|---|---|---|---|
| `auto_scrape` *(v1.1.0+; v1.2.0 adds `hostile_only` + `pattern_d_network_idle` + `is_structured`; v1.3.0 adds `persist_browser_profile_dir`)* | `url, schema_json?, instruction?, model?, browser?, timeout_s?, hostile_only?, hostile_fallback?, pattern_d_network_idle?, persist_browser_profile_dir?` | `{pattern_used, pattern_attempts, product?, data?, blocked, hostile_skipped, is_structured, ...}` | **Recommended first tool.** Auto-escalates Pattern A/B/C → D → E1 → E2. By default v1.3.0+ allocates a per-request ephemeral browser profile dir so D's CF clearance carries forward to E1/E2. Set `persist_browser_profile_dir` for cross-request reuse on poll-style workloads (caller owns lifecycle + per-vendor isolation + ~30min rotation). |
...
2. **The lib does not bundle authentication**, and the agent cannot acquire it. If a fetch needs a logged-in session, the *user* exports one on their own machine with `scrapper-tool cookies export --domain <host>` and it is passed in as `cookies=` (library / `POST /scrape`) — see [Settings](SETTINGS.md#the-cascade-does-not-remember-your-login-between-requests). Cookie **extraction** is deliberately absent from the MCP surface: an agent that can silently dump a user's browser cookie store is exactly the capability not to build, and a consent prompt means nothing when the caller is a model. MCP tools *consume* cookies handed to them; they never read the browser. For OAuth tokens and other bearer credentials, wire them via `extra_headers` on the underlying `httpx` client. Don't tunnel secrets through the URL.
