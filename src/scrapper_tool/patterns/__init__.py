...
- ``b`` — Embedded JSON (LD+JSON / ``__NEXT_DATA__`` / ``__NUXT__`` /
  ``self.__next_f.push``). ``patterns.b`` (extruct-backed).
- ``c`` — CSS / schema.org microdata. ``patterns.c`` (selectolax-backed).
- ``d`` — Hostile (Cloudflare Turnstile / Akamai EVA / etc.).
  ``patterns.d`` (Scrapling-backed; ``pip install scrapper-tool[hostile]``).
